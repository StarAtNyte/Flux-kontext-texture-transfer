...
def infer(input_images, prompt, seed=42, randomize_seed=False, guidance_scale=2.5, progress=gr.Progress(track_tqdm=True)):
    ...
    # Use dimensions of the second image if available, otherwise use concatenated image dimensions
    if len(valid_images) >= 2:
        # Use dimensions from the second image
        target_width = valid_images[1].size[0]
        target_height = valid_images[1].size[1]
    else:
        # Fallback to concatenated image dimensions
        target_width = concatenated_image.size[0]
        target_height = concatenated_image.size[1]
            ...
            with gr.Column():
                ...
                with gr.Row():
                    prompt = gr.Text(
                        ...
                        max_lines=1,
                        placeholder="Apply the texture from the first image to the sofa in the second image. Give me only the sofa with the new design",
                        container=False,
                    )
